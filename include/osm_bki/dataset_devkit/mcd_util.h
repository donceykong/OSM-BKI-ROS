...
class MCDData {
  ...
  public:
    ...
    bool process_scans(std::string input_data_dir, std::string input_label_dir, int scan_num, double keyframe_dist, bool query, bool visualize) {
      ...
      // Apply scan_num and keyframe_dist to the valid set: select scans whose pose is at least
      // keyframe_dist (Euclidean) from the last selected scan's pose, up to scan_num.
