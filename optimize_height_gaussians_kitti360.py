#!/usr/bin/env python3
"""Optimize per-class Gaussian height priors (mu, tau) for KITTI-360 GT data.

Mirror of optimize_height_gaussians_mcd.py, adapted for KITTI-360:
  * pose_format = "kitti360" (velodyne_poses.txt -> load_poses_kitti360)
  * no body-frame calibration (poses are already in the lidar/world frame,
    so lidar_to_body = I)
  * label .bin files may be uint16 or uint32 — resolved via
    read_label_bin(..., n_points_hint=n).

The produced (mu, tau) pair is meant to be pasted directly into
`config/methods/kitti360.yaml` under `height_kernel_mu` / `height_kernel_tau`
(with `height_filter_type: gaussian`).

The height reference matches the C++ runtime
...
Output: a YAML at config/datasets/height_gaussians_kitti360.yaml containing
...
from optimize_osm_cm_kitti360 import (  # noqa: E402
    CLASS_NAMES,
    N_CLASSES,
    load_poses,
    load_label_mappings,
...
def resolve_paths(cfg, data_dir_override):
    """Build absolute paths from sequence_name + *_suffix (same convention as
    optimize_osm_cm_kitti360.py)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize per-class Gaussian height priors (mu, tau) for KITTI-360 GT.")
    parser.add_argument("--config",
                        default=os.path.join(SCRIPT_DIR, "config/methods/kitti360.yaml"))
    parser.add_argument("--output",
                        default=os.path.join(SCRIPT_DIR,
                                             "config/datasets/height_gaussians_kitti360.yaml"))
    ...
    parser.add_argument("--max-points-per-class-per-scan", type=int, default=200,
                        help="Stratify: cap samples per class *per scan* so a dense scan "
                             "can't dominate the distribution (0 = unlimited).")
    parser.add_argument("--max-points-per-class", type=int, default=5000,
    ...
    ds_resolution = cfg.get("ds_resolution", 0.5)
    ...
    poses = load_poses(pose_file, pose_format="kitti360")
    ...
    # KITTI-360: velodyne poses are already in the sensor/world frame, so
    # there is no body->lidar transform.
    ...
    for (idx, T, scan_path, label_path) in tqdm(scan_list, desc="scans", unit="scan"):
        ...
        for c in range(N_CLASSES):
            ...
            if cap > 0 and h_c.size > cap:
                idx_s = rng.choice(h_c.size, size=cap, replace=False)
                h_c = h_c[idx_s]
    ...
    out = {
        ...
        # Ready-to-paste into config/methods/kitti360.yaml under ros__parameters:
