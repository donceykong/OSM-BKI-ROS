...
Identical methodology to optimize_height_dirichlet_mcd.py — see that script for full
method description. Key differences from the MCD variant:
  - Imports helpers from optimize_osm_cm_kitti360 (supports kitti360 pose format).
  - KITTI-360 velodyne poses are already in sensor/world frame → identity calibration.
  - Default config is config/methods/kitti360.yaml.
  - Output is config/datasets/height_dirichlet_kitti360.yaml.
...
def main():
    parser = argparse.ArgumentParser(
        description="Per-class height kernel parameters via Dirichlet posterior (KITTI-360).")
    parser.add_argument("--config",
                        default=os.path.join(SCRIPT_DIR, "config/methods/kitti360.yaml"))
    parser.add_argument("--output",
                        default=os.path.join(SCRIPT_DIR,
                                             "config/datasets/height_dirichlet_kitti360.yaml"))
    ...
    parser.add_argument("--plot", action="store_true",
                        help="Save per-class posterior plots to height_dirichlet_kitti360_plots/.")
    ...
    pose_format = cfg.get("pose_format", "kitti360")
    ...
    print(f"pose_format             = {pose_format}")
    ...
    # KITTI-360 poses are already in sensor frame → identity calibration
    lidar_to_body = np.eye(4, dtype=np.float64)
    print("Using identity calibration (KITTI-360 poses already in sensor frame)")
    ...
    out = {
        ...
        "height_kernel_mu":        mu_list,
        "height_kernel_tau":       tau_list,
        "height_kernel_dead_zone": dz_list,
    ...
    if args.plot:
        ...
        plot_dir = os.path.join(os.path.dirname(args.output), "height_dirichlet_kitti360_plots")
        ...
        for c in range(N_CLASSES):
            ...
            h_plot = np.linspace(args.h_min, args.h_max, 2000)
            phi = np.array([np.exp(-(max(0.0, abs(hi - mu) - dz) ** 2) / (2 * tau ** 2))
                            for hi in h_plot])
